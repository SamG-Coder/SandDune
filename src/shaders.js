...
float wetness(vec2 p) { return clamp(sampleGrid(uMoisture,p/160.0+0.5,uSandResolution)/0.55,0.0,1.0); }
float waterDepth(vec2 p) { return sampleGrid(uWater,p/160.0+0.5,uSandResolution); }
...
uniform vec3 uBrush;
uniform float uWaterTool;
varying vec3 vWorld;
void main(){
 vec2 p=vWorld.xz;
 float wet=wetness(p);
 float mud=smoothstep(.65,.95,wet);
 ...
 albedo*=1.0-wet*.42;
 albedo=mix(albedo,vec3(.18,.105,.055),mud*.62);
 vec3 color=albedo*(ambient+sunlight*ndl*shade);
 vec3 viewDirection=normalize(cameraPosition-vWorld);
 vec3 halfway=normalize(viewDirection+uSun);
 color+=vec3(.85,.94,1.0)*pow(max(dot(n,halfway),0.0),64.0)*wet*.34*shade;
 ...
 color=mix(color,uFog,veil*pow(1.0-wet,3.0));
 float fog=1.0-exp(-distanceToCamera*uHaze*.00025);
 color=mix(color,uFog,min(fog,.97));
 if(uBrush.z>0.0){float r=length(p-uBrush.xy);float line=1.0-smoothstep(.025,.09,abs(r-uBrush.z));color=mix(color,mix(vec3(.95,.83,.62),vec3(.35,.75,1.0),uWaterTool),line*.7);}
