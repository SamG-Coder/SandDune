...
float terrain(vec2 p) {
  float distant=sampleGrid(uBaseHeight,p/1400.0+0.5,513.0);
  if(max(abs(p.x),abs(p.y))>=80.0)return distant;
  float local=sampleGrid(uSandHeight,p/160.0+0.5,uSandResolution);
  float blend=1.0-smoothstep(75.0,79.0,max(abs(p.x),abs(p.y)));
  return mix(distant,local,blend);
}
...
void main(){
 ...
 float e=mix(0.6,4.5,smoothstep(65.0,95.0,max(abs(p.x),abs(p.y))));
 ...
 float fog=1.0-exp(-distanceToCamera*(.0008+uHaze*.004));
...
uniform vec3 uSun;uniform vec3 uFog;uniform float uHaze;varying vec3 vDirection;
void main(){vec3 ray=normalize(vDirection);float elevation=max(ray.y,0.0);vec3 zenith=mix(vec3(.25,.38,.5),vec3(.2,.4,.6),uSun.y);zenith=mix(zenith,uFog,uHaze*.7);vec3 color=mix(uFog,zenith,1.0-exp(-elevation*4.0));float alignment=max(dot(ray,uSun),0.0);color+=vec3(1,.69,.35)*pow(alignment,12.0)*.16;color+=vec3(1,.77,.45)*pow(alignment,180.0)*.24;color+=vec3(3,2.4,1.55)*smoothstep(cos(.009),cos(.006),alignment)*(1.0-uHaze*.8);gl_FragColor=vec4(color,1);
#include <tonemapping_fragment>
#include <colorspace_fragment>
}
`;
...
void main(){vec2 p=mod(position.xz+uDrift*(1.0+aSeed.x*.9)+vec2(100),vec2(200))-100.0;float hop=.5+.5*sin(uTime*(1.8+aSeed.x*2.0)+aSeed.y*30.0);float altitude=.04+hop*hop*(.1+uWind*.04)*aSeed.z;vec4 mv=viewMatrix*vec4(p.x,terrain(p)+altitude,p.y,1);vAlpha=(1.0-smoothstep(75.0,99.0,max(abs(p.x),abs(p.y))))*smoothstep(3.0,15.0,uWind)*(.16+aSeed.w*.2)*(1.0-smoothstep(60.0,160.0,-mv.z));gl_PointSize=clamp((18.0+aSeed.w*30.0)*uPixelRatio/max(-mv.z,1.0),1.0,4.0);gl_Position=projectionMatrix*mv;}
`;
